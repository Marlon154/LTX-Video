...
# Adapted from: https://github.com/Lightricks/LTX-Video/blob/main/ltx_video/pipelines/pipeline_ltx_video.py
import html
import inspect
import math
import re
...
class SemanticLTXVideoPipeline(DiffusionPipeline):
    r"""
    ...
    def __call__(
        ...
        **kwargs,
        ...
        Args:
            ...
            pooled_editing_prompt_embeds (`torch.Tensor`, *optional*):
                Pre-computed pooled embeddings to use for semantic guidance.
        ...
        if enable_edit_guidance and editing_prompt_embeddings is None:
            ...
            new_editing_prompt_embeddings = []
            new_attention_mask_editing_prompt_embeds = []
            for c in editing_prompt:
                (
                    editing_embeds,
                    edit_prompt_attention_mask,
                    _,
                    _,
                ) = self.encode_prompt(
                    c,
                    ...
                    num_images_per_prompt=num_images_per_prompt,
                    device=device,
                    prompt_embeds=editing_prompt_embeddings,
                    prompt_attention_mask=editing_prompt_attention_mask,
                    clean_caption=clean_caption,
                )
                new_editing_prompt_embeddings.append(editing_embeds)
                new_attention_mask_editing_prompt_embeds.append(edit_prompt_attention_mask)
            editing_prompt_embeddings = torch.cat(new_editing_prompt_embeddings, dim=0)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if do_classifier_free_guidance:
                    ...
                    #     self.text_estimates = torch.zeros((num_steps + 1, *noise_pred_text.shape))
                    # self.text_estimates[i] = noise_pred_text.detach().cpu()

                    if self.edit_estimates is None and enable_edit_guidance:
                        self.edit_estimates = torch.zeros(
                            (num_steps + 1, len(noise_pred_edit_concepts), *noise_pred_edit_concepts[0].shape)
                        )

                    if self.sem_guidance is None:
                        self.sem_guidance = torch.zeros((num_steps + 1, *noise_pred_text.shape))
